...
def sample_strings(length, count):
    ...
    return [''.join(
        random.sample(string.ascii_lowercase, length)).encode() for i in range(count)]
...
class ErrorsTest(BaseTest):
    ...
    def test_error(self):
        ...
        self.assertRaises(pyreBloomException, self.bloom.extend, [b'a', b'b'])
        self.assertRaises(pyreBloomException, self.bloom.contains, b'a')
        self.assertRaises(pyreBloomException, self.bloom.contains, [b'a', b'b'])
